...
data_path = '/home/arvind/Downloads/' # Directory where all the collected data is
check_path = '/home/arvind/CMU/MAIL/VINN/VINN-Main/Data/PointClouds/Line_all/Line_PC_data/run_pc_2/images/0004.npy'
save_path = '/home/arvind/CMU/MAIL/VINN/VINN-Main/Data/PointClouds/Line_all/Line_PC_data/run_pc_'
...
val = np.load(check_path)
checkPC = o3d.geometry.PointCloud()
# print(np.mean(val, axis =0))
checkPC.points = o3d.utility.Vector3dVector(val)
checkPC.colors = o3d.utility.Vector3dVector(np.tile(np.array([1, 0, 0]), (len(val), 1)))

for j in range(n_rot_aug):
    points = np.load(data_path + 'state72.npy')
    center = np.mean(points, axis = 0)
    rot  = 360/n_rot_aug*j
    points = augment_state_action(points, None, center, None, rot, False)
    points = (points - center) * 10
    print(j)
    np.save(save_path + str(j*10+2) + '/images/0005.npy', points)
    ...
    # o3d.visualization.draw_geometries([pointcloud, checkPC])
# for k in range(n_rot_aug):
